...
func TestE2EBuildLevel3(t *testing.T) {
	...
	result := app.RunBuild(context.Background(), srcDir, outDir, "", 3, false, false, false)
...
func TestE2EBuildInvalidSource(t *testing.T) {
	...
	result := app.RunBuild(context.Background(), "/nonexistent/path", outDir, "", 1, false, false, false)
	assert.False(t, result.Success)
...
func TestE2EBuildJSONOutput(t *testing.T) {
	...
	result := app.RunBuild(context.Background(), srcDir, outDir, "", 1, false, true, false)
	assert.True(t, result.Success)
...
func TestE2EBuildExistingWikiIdempotent(t *testing.T) {
	...
	r1 := app.RunBuild(context.Background(), srcDir, outDir, "", 1, false, false, false)
	r2 := app.RunBuild(context.Background(), srcDir, outDir, "", 1, false, false, false)
...
func TestE2EInfo(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(srcDir, "doc.md"), []byte("# Doc"), 0644))
	_ = app.RunBuild(context.Background(), srcDir, outDir, "", 1, false, true, false)
