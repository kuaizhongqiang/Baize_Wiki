...
func TestEdgeInfoStatsFlag(t *testing.T) {
	...
	_ = RunBuild(context.Background(), srcDir, outDir, "", 1, false, false)
...
func TestEdgeOutputPathInvalid(t *testing.T) {
	...
	result := RunBuild(context.Background(), srcDir, conflictPath, "", 1, false, false)
...
func TestEdgeBuildWithDraftFlag(t *testing.T) {
	...
	outDir2 := t.TempDir()
	resultWithDraft := RunBuild(context.Background(), srcDir, outDir2, "", 1, true, false)
